...
/**
Mini Project, use every query seen in the lesson for data digging in the sample data table
**/
...
SELECT Title, max(Rank) AS "Most_Liked" FROM topmovies;
SELECT avg(Rank) AS "AVG_GRADE" FROM topmovies;
SELECT Title, min(Rank) AS "Most_Disliked" FROM topmovies;

SELECT Studio, Round(avg(Rank)) AS "AVG_STUDIO", Count() AS "Number_movies" FROM topmovies Group By Studio HAVING AVG_STUDIO>50 ORDER BY AVG_STUDIO;

SELECT Studio, Count(*) AS "Number_movies",
    CASE
        WHEN Round(avg(Rank)) > 90 THEN "Masterpieces"
        WHEN Round(avg(Rank)) > 80 THEN "Great movies"
        WHEN Round(avg(Rank)) > 70 THEN "Good movies"
        WHEN Round(avg(Rank)) > 60 THEN "Avg movies"
        WHEN Round(avg(Rank)) > 50 THEN "Not that great movies"
        ELSE "Bad Movies"
    END AS "Studio_grade"
FROM topmovies GROUP BY Studio ORDER BY Number_movies;

SELECT Title, Rank, Year FROM topmovies WHERE Rank > 70 AND Year > 2007;

SELECT Title, Domestic, Overseas FROM topmovies WHERE Domestic <= 170 OR Overseas <= 180;
